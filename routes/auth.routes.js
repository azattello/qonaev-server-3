...
router.post('/registration', 
    [
        ...
        check('selectedFilial', 'Филиал обязателен для заполнения')
    ],
    async (req, res) => {
    try {
        ...
        const { phone, password, name, surname, selectedFilial } = req.body;
        ...
        // Поиск максимального значения personalId
        const lastUser = await User.findOne().sort({ personalId: -1 });
        const newPersonalId = lastUser ? lastUser.personalId + 1 : 96;

        // Создание нового пользователя с personalId
        const user = new User({
            phone,
            password: password,
            name,
            surname,
            selectedFilial,
            personalId: newPersonalId, // Устанавливаем personalId
            createdAt: new Date()
        });

        await user.save();
        return res.json({ message: "Пользователь создан с personalId: " + newPersonalId });
